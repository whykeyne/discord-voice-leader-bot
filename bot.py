...
# Custom server emojis exactly as provided by the user.
EMOJI = {
...
async def remove_voice_flag_later(member: discord.Member, flag: str, delay_seconds: int) -> None:
    await discord.utils.sleep_until(datetime.now(timezone.utc) + discord.utils.timedelta(seconds=delay_seconds))


# Fallback because discord.utils.timedelta does not exist in discord.py.
...
# override helper above with working scheduler wrappers
async def remove_voice_flag_later(member: discord.Member, flag: str, delay_seconds: int) -> None:
    if flag == "mute":
...
class RoomPanelView(discord.ui.View):
    ...
    async def mic_btn(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        ...
        if not await ensure_control_rights(interaction, channel):
            return
        await interaction.response.send_message("Выбери участника:", ephemeral=True, view=MemberActionView(bot, channel.id, "mute", interaction.user.id))
