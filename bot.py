...
async def rename_btn(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        if not await ensure_control_rights(interaction, channel):
            return
        try:
            await channel.edit(name=str(self.new_name), reason=f"Rename by {interaction.user}")
            await sync_room_panel(channel)
            await safe_send(interaction, f"Название комнаты изменено на **{self.new_name}**.")
        except discord.Forbidden:
            await safe_send(interaction, "Боту не хватает права Manage Channels.")
        except discord.HTTPException:
            await safe_send(interaction, "Не удалось переименовать комнату.")
...
class RoomPanelView(discord.ui.View):
    ...
    @discord.ui.button(label="Переименовать", style=discord.ButtonStyle.secondary, emoji="✏️", row=1, custom_id="room_rename")
    async def rename_btn(self, interaction: discord.Interaction, _: discord.ui.Button) -> None:
        channel = self._get_channel(interaction)
        if not channel:
            await safe_send(interaction, "Комната уже недоступна.")
        ...
        if not await ensure_control_rights(interaction, channel):
            return
        await interaction.response.send_modal(RenameRoomModal(channel.id, channel.name))
